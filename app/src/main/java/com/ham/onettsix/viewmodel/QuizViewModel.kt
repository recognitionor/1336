...
class QuizViewModel(
    ...
    val quizTagList = MutableLiveData<Resource<ArrayList<QuizTag.Data>>>()
    ...
    fun getQuizTag() {
        val exceptionHandler = CoroutineExceptionHandler { _, e ->
            quizTagList.postValue(Resource.error(e.toString(), null))
        }
        viewModelScope.launch(exceptionHandler) {
            val list = arrayListOf<QuizTag.Data>()
            list.add(QuizTag.Data(1, "경제 & 재테크"))
            list.add(QuizTag.Data(2, "일반 상식"))
            list.add(QuizTag.Data(3, "기타"))
            list.add(QuizTag.Data(4, "역사"))
            quizTagList.postValue(Resource.success(list))
        }
    }
